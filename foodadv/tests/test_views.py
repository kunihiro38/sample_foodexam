...
食品表示検定・初級テスト
全体テスト
python manage.py test foodadv.tests
クラス単位のテスト 例
python manage.py test foodadv.tests.test_views.TestRegularTests
...
from foodadv.forms import SelectAnswerForm, SearchQuestionForm
from foodadv.models import FoodLabelingAdviseQuestion, FoodadvRecord, FoodadvTimeLeft, AnswerResult
...
class TestRegularTests(TestCase):
    ...
    def setUp(self):
        ...
        df = pd.read_csv(f'foodadv/csv/{csv_file}')
        ...
        for _, row in df_replace_non.iterrows():
            ...
            foodadv_objects.append(FoodLabelingAdviseQuestion(**dict_data))
        FoodLabelingAdviseQuestion.objects.bulk_create(foodadv_objects) # 一括作成
    ...
    def test_practice_is_true(self):
        ...
        res = client_for_alex.post(
            path='/foodadv/signin/?next=/foodadv/',
            data=login_user_data,
        )
        self.assertEqual(res.status_code, 302)
        res = client_for_alex.get(path='/foodadv/')
        self.assertEqual(res.status_code, 200)
        res = client_for_alex.get(path='/foodadv/profile/')
        self.assertEqual(res.status_code, 200)
        ...
        for question_id in question_qs_ids:
            ...
            record_objects.append(FoodadvRecord(**data))
        FoodadvRecord.objects.bulk_create(record_objects)

        res = client_for_alex.get(path='/foodadv/record/list/')
        self.assertEqual(res.status_code, 200)
        ...
        # 問題IDにアクセスできる
        latest_record_qs = FoodadvRecord.objects.filter(user_id=user_alex.id).latest('saved_at')
        latest_question_id = latest_record_qs.question_id
        self.assertIn(latest_question_id, res.content.decode())

        res = client_for_alex.get(path=f'/foodadv/record/question_id/{latest_question_id}')
        self.assertEqual(res.status_code, 200)
        self.assertIn(latest_question_id, res.content.decode())
        ...
        res = client_for_alex.get(path='/foodadv/record/list/?page=2')
        self.assertEqual(res.status_code, 200)

        # メモページ
        res = client_for_alex.get(path='/foodadv/record/memos/')
        self.assertEqual(res.status_code, 200)
        self.assertIn('登録されているメモはありません。', res.content.decode())

        # 全てのレコードにメモを追加
        record_qs = FoodadvRecord.objects.filter(user_id=user_alex.id)
        for record in record_qs:
            record.memo = 'TRUE!!'
        FoodadvRecord.objects.bulk_update(record_qs, fields=['memo'])
        ...
        self.assertIn('?page=2', res.content.decode())
        # 2ページ目も問題なく表示されていることを確認
        res = client_for_alex.get(path='/foodadv/record/memos/?page=2')
        self.assertEqual(res.status_code, 200)

        # お気に入りページ
        res = client_for_alex.get(path='/foodadv/record/favorites/')
        self.assertEqual(res.status_code, 200)
        ...
        FoodadvRecord.objects.bulk_update(record_qs, fields=['favorite'])
        ...
        self.assertIn('?page=2', res.content.decode())
        # 2ページ目も問題なく表示されていることを確認
        res = client_for_alex.get(path='/foodadv/record/favorites/?page=2')
        self.assertEqual(res.status_code, 200)

        # 学習履歴の削除
        res = client_for_alex.get(path='/foodadv/setting/')
        self.assertEqual(res.status_code, 200)
        res = client_for_alex.get(path='/foodadv/setting/del_record/')
        self.assertEqual(res.status_code, 200)
        ...
        res = client_for_alex.post(
            path='/foodadv/setting/del_record/',
        ...
        # 削除後画面遷移する
        res = client_for_alex.get(path='/foodadv/setting/del_record/success')
        self.assertEqual(res.status_code, 200)
        ...
        # 問題検索ページにアクセスできる
        res = client_for_alex.get(path='/foodadv/search/')
        self.assertEqual(res.status_code, 200)

        # 問題IDにquestion_type='label'の画像を含むidを取得して検索にかける
        target_ids_include_img = FoodLabelingAdviseQuestion.objects.filter(question_type='label')
        ...
        res = client_for_alex.get(
            path=f'/foodadv/search/question_id/{target_id_include_type_label}')
        self.assertEqual(res.status_code, 200)
        ...
        target_ids_include_img = FoodLabelingAdviseQuestion.objects.filter(question_type='img')
        ...
        # が含まれていることを確認する
        res = client_for_alex.get(
            path=f'/foodadv/search/question_id/{target_id_include_type_img}')
        self.assertEqual(res.status_code, 200)
        ...
        target_ids_include_img = FoodLabelingAdviseQuestion.objects.filter(question_type='surround')
        ...
        # が含まれていることを確認する
        res = client_for_alex.get(
            path=f'/foodadv/search/question_id/{target_id_include_type_surround}')
        self.assertEqual(res.status_code, 200)
    ...
    def test_answer_only_one_regular_true(self):
        ...
        ### 3. 確認ページで未解答であることを確認
        res = client_for_alex.get(path='/foodadv/practice_finish_confirm/')
        self.assertEqual(res.status_code, 200)
        ...
        ### 5. 正答を解答する
        # 正答をgetする
        question = FoodLabelingAdviseQuestion.objects.get(question_id=get_decoded_question_id)
        ...
        ### 7. 確認ページで答え合わせをする
        res = client_for_alex.post(path='/foodadv/practice_finish_confirm/')
        self.assertEqual(res.status_code, 302)

        ### 8. 演習結果
        res = client_for_alex.get(path='/foodadv/practice/result')
        self.assertEqual(res.status_code, 200)
    ...
    def test_answer_three_regular(self):
        ...
        def _get_correct_answer(page_question_id):
            """問題IDを渡して正答を返す"""
            question = FoodLabelingAdviseQuestion.objects.get(question_id=page_question_id)
            correct_answer = question.correct_answer
        ...
        res = client_for_alex.get(
            path=f'/foodadv/back_to_last_page/{page3_question_id}?back_button=',
        )
        self.assertEqual(res.status_code, 302)
        url = res.url
    ...
    def test_practice_test_one_regular(self):
        ...
        client_for_alex = Client()
        res = client_for_alex.post(
            path='/foodadv/signin/?next=/foodadv/select_test/',
            data=user_data,
        )
        self.assertEqual(res.status_code, 302)

        # 解答状況は模擬試験実施前は404になる
        res = client_for_alex.get(path='/foodadv/practice_test/answer_status')
        self.assertEqual(res.status_code, 404)
        # 答え合わせ前確認は模擬試験実施前は404になる
        res = client_for_alex.get(path='/foodadv/practice_test_finish_confirm/')
        self.assertEqual(res.status_code, 404)
        ...
        res = client_for_alex.get(
            path='/foodadv/select_test/',
            data=choiced_data,
        )
        self.assertEqual(res.status_code, 302)

        # 1ページ目
        res = client_for_alex.get(path='/foodadv/practice_test/',)
        self.assertEqual(res.status_code, 200)

        # 未解答でPOSTする
        res = client_for_alex.post(path='/foodadv/practice_test/',)
        self.assertEqual(res.status_code, 302)

        # 2ページ目
        res = client_for_alex.get(path='/foodadv/practice_test/?page=2',)
        self.assertEqual(res.status_code, 200)
        ...
        res = client_for_alex.post(
            path='/foodadv/practice_test/?page=2',
            data=choiced_answer,
        )
        self.assertEqual(res.status_code, 302)
        ...
        # 3ページ目
        # 未解答ですすむ
        res = client_for_alex.get(path='/foodadv/practice_test/?page=3')
        self.assertEqual(res.status_code, 200)

        # 4ページ目以降は、4-75問をforで処理
        for _ in range(4,76):
            url = f'/foodadv/practice_test/?page={str(_)}'
            res = client_for_alex.get(path=url)
            self.assertEqual(res.status_code, 200)
            # ページから問題IDを取得
            get_decoded_question_id = re.findall(r'(?<=data-question-id=")j[0-9]{1,6}',
        ...
        res = client_for_alex.post(path='/foodadv/practice_test_finish_confirm/')
        self.assertEqual(res.status_code, 302)
    ...
    def test_rechallenge_mistakes(self):
        ...
        foodadv_qs = FoodLabelingAdviseQuestion.objects.all()
        question_ids = [foodadv.question_id for foodadv in foodadv_qs]
        ...
        for question_id in question_ids:
            ...
            record_data.append(FoodadvRecord(**data))
        # record一括作成
        FoodadvRecord.objects.bulk_create(record_data)
        self.assertEqual(len(question_ids), FoodadvRecord.objects.count())
        ...
        res = client_for_alex.get(
            path='/foodadv/practice/rechallenge_mistakes',
            data=button_name)
        # 302遷移
        self.assertEqual(res.status_code, 302)
    ...
    def test_practice_test_two_regular(self):
        ...
        record_qs = FoodadvRecord.objects.filter(question_id__in=duplicated_ids)
    ...
    def test_practice_test_many_times(self):
        ...
        all_questions = FoodLabelingAdviseQuestion.objects.count()
        answers = 75 # 模擬試験の解答数
        # 何回まで未解答の問題に遭遇するか
    ...
    def tearDown(self):
        ...
        FoodadvRecord.objects.all().delete()
        FoodadvTimeLeft.objects.all().delete()
        FoodLabelingAdviseQuestion.objects.all().delete()
