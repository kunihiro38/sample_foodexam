...
from foodadv.forms import DelFoodadvRecordForm, SelectQuestions, SelectQuestionForm, \
    SelectPracticeTestForm, SearchQuestionForm, GetPageNumForm, SelectAnswerForm, \
    SearchFoodadvRecord, _make_choice_combination
from foodadv.models import AnswerResult, FoodadvRecord, FoodLabelingAdviseQuestion, \
    FoodadvTimeLeft, TextbookChapter
...
def check_auth(func):
    ...
    #     if user_course.payment_course != PaymentCourse.FOODADV:
    #         return HttpResponseRedirect(reverse('corporate:foodadv_plan'))
...
def signin(request):
    ...
    if request.method == 'GET':
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                return HttpResponseRedirect(reverse('foodadv:index'))
            else:
                pass
    context = {
        'form': form
...
def profile(request):
    ...
    user_info_dict = {
        ...
        'email': user.email,
        'payment_course': user_course.foodadv_payment_course_as_str(),
        'paid_at': user_course.foodadv_paid_at,
        'expired_at': user_course.foodadv_expired_at,
    }
    # ログアウト処理
    ...
    return render(request, 'foodadv/profile/profile.html', context)
...
def setting(request):
    """設定"""
    return render(request, 'foodadv/foodadv_setting/setting.html')
...
def del_record(request):
    ...
    if request.method == 'GET':
        form = DelFoodadvRecordForm(user=request.user)
    elif request.method == 'POST':
        form = DelFoodadvRecordForm(user=request.user, data=request.POST)
        if form.is_valid():
            username = form.cleaned_data['username']
            if username != request.user.username:
                raise RuntimeError()
            foodadv_logics.delete_foodadv_record_by_user_id(user_id=request.user.id)
            return HttpResponseRedirect(reverse('foodadv:del_record_success'))
    context = {
        'form': form,
        'plan': plan,
    }
    return render(request, 'foodadv/foodadv_setting/del_record.html', context)
...
def del_record_success(request):
    """学習履歴の削除の成功"""
    return render(request, 'foodadv/foodadv_setting/del_record_success.html')
...
def count_question_ajax(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        # 全問題数カウント
        question_qs = FoodLabelingAdviseQuestion.objects.all()
        record_qs = FoodadvRecord.objects.filter(user_id=user_id)
        ...
        questions_dict = {'LAW_SYSTEM': 0,
                            'FRESH_FOOD': 0,
                            'PROCESSED_FOOD': 0,
                            'LABEL_EXAMPLE': 0,
                            'INDIVIDUAL_COMMENTARY': 0,
                            'NUTRITION_LABEL': 0,
                            'all_questions': 0}

        for question in question_qs:
            if question.textbook_chapter == TextbookChapter.LAW_SYSTEM:
                questions_dict['LAW_SYSTEM'] += 1
            ...
            elif question.textbook_chapter == TextbookChapter.LABEL_EXAMPLE:
                questions_dict['LABEL_EXAMPLE'] += 1
            elif question.textbook_chapter == TextbookChapter.INDIVIDUAL_COMMENTARY:
                questions_dict['INDIVIDUAL_COMMENTARY'] += 1
            elif question.textbook_chapter == TextbookChapter.NUTRITION_LABEL:
                questions_dict['NUTRITION_LABEL'] += 1
            else:
                raise RuntimeError()
...
def _get_latest_record(request):
    """前回の演習記録取得"""
    latest_record = False
    if FoodadvRecord.objects.filter(user_id=request.user.id).exists():
        latest_record = FoodadvRecord.objects.filter(user_id=request.user.id).latest('saved_at')
...
def _check_one_question_a_day(request, plan):
    ...
    if not plan:
        ...
        if FoodadvRecord.objects.filter(user_id=request.user.id).exists():
            ...
            if FoodadvRecord.objects.filter(user_id=request.user.id)\
                ...
                # recordが本日中のものか判定
                record_qs = FoodadvRecord.objects.filter(user_id=request.user.id)\
                                    .filter(Q(current_answer=AnswerResult.UNANSWERED)\
                                        |Q(current_answer=AnswerResult.INCORRECT)\
                                        |Q(current_answer=AnswerResult.CORRECT)
                                        ).latest('updated_at')
...
def index(request):
    ...
    return render(request, 'foodadv/index.html', context)
...
def select_test(request):
    ...
    if form.is_valid():
        ...
        if 'start_button' in request.GET:
            ...
            # 残り時間作成
            time_left_qs = FoodadvTimeLeft.objects.filter(user_id=request.user.id)
            dt_now = datetime.datetime.now()
            ...
            if time_left_qs:
                ...
                time_left.time_left_at = time_left
            else:
                # 初めて模擬試験を受ける場合は、新規オブジェクト作成
                time_left = FoodadvTimeLeft(user_id=request.user.id,
            ...
            return HttpResponseRedirect(reverse(
                'foodadv:practice_test'))
    context = {
        'form': form,
        'latest_record': latest_record,
        'plan': plan
    }
    return render(request, 'foodadv/select_test.html', context)
...
def _create_questions(request, target, chapter, answers):
    ...
    if target:
        ...
        if str(SelectQuestions.UNQUESTIONED) in target and \
            ...
            question_qs = FoodLabelingAdviseQuestion.objects.exclude(
        ...
        # 2. ミスのみにチェック→ミスだけ抽出して検索
            ...
            record_qs = record_qs.filter(current_answer=AnswerResult.INCORRECT)
            record_question_ids = [record.question_id for record in record_qs]
            question_qs = FoodLabelingAdviseQuestion.objects.filter(
                question_id__in=record_question_ids)
    ...
    # 出題対象(未出題・ミス)が何も選択されていない、出題分野だけ選択された場合
        ...
        question_qs = FoodLabelingAdviseQuestion.objects\
...
def practice(request, get_params):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if 'question_id_review_flg' in request.session:
                return HttpResponseRedirect(reverse('foodadv:practice_finish_confirm'))
                        ...
                        if page_num < len(session_question_ids_list):
                            ...
                            if page_num+1 == len(session_question_ids_list):
                                return HttpResponseRedirect(
                                    reverse('foodadv:practice_finish_confirm'))
    ...
    return render(request, 'foodadv/practice/practice.html', context)
...
def _save_memo(user_id, question_id, memo):
    ...
    idあり→更新
    """
    record_qs = FoodadvRecord.objects.filter(question_id=question_id,
                                    user_id=user_id,)
    if not record_qs: # 新規
        memo_record = FoodadvRecord(question_id=question_id,
                                user_id=user_id,
                                memo=memo,)
    elif record_qs: # 更新
...
def _save_favorite(user_id, question_id):
    ...
    record_qs = FoodadvRecord.objects.filter(question_id=question_id,
                                    user_id=user_id,)
    if not record_qs: # 新規
        favorite_record = FoodadvRecord(question_id=question_id,
                                    user_id = user_id,
                                    favorite = True,)
...
def _save_later(user_id, question_id):
    ...
    record_qs = FoodadvRecord.objects.filter(question_id=question_id,
                                    user_id=user_id,)
    if not record_qs: # 新規
        later_record = FoodadvRecord(question_id=question_id,
                                user_id = user_id,
                                later = True,)
...
def _answer_result(question_id, select_answer):
    """正誤判断"""
    question_qs = FoodLabelingAdviseQuestion.objects.get(question_id=question_id)
    if select_answer == '':
...
def _save_record(request, question_info):
    ...
    for question in question_info:
        ...
        if question['question_id'] not in record_qs_ids:
            ...
            if not FoodadvRecord.objects.filter(user_id=request.user.id,
                                            question_id=question['question_id']).exists():
                new_record = FoodadvRecord(question_id=question['question_id'],
                                    user_id=request.user.id,
...
def practice_finish_confirm(request):
    ...
    if request.method == 'POST':
        ...
        return HttpResponseRedirect(reverse('foodadv:practice_result'))
    ...
    return render(request, 'foodadv/practice/practice_finish_confirm.html', context)
...
def practice_result(request):
    ...
    return render(request, 'foodadv/practice/result.html', context)
...
def practice_result_individual(request, question_id):
    ...
    get_question = get_object_or_404(FoodLabelingAdviseQuestion, question_id=question_id)
    get_record = get_object_or_404(FoodadvRecord, user_id=request.user.id, question_id=question_id, )
    ...
    return render(request, 'foodadv/practice/result_individual.html', context)
...
def _create_test_questions(request):
    ...
    if 'unquestioned_and_mistakes_flg' in request.session:
        ...
        if question_qs.count() < ANSWERS:
            ...
            sufficient_qs = FoodLabelingAdviseQuestion.objects\
...
def _make_strftime_time_left(request):
    get_time_left = get_object_or_404(FoodadvTimeLeft, user_id=request.user.id)
...
def practice_test(request):
    """模擬試験"""
    _check_form_and_url_session(request)
    get_object_or_404(FoodadvTimeLeft, user_id=request.user.id)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # →　practice_finish_confirm/のquestion_idのaタグから来た場合
            if 'question_id_review_flg' in request.session:
                return HttpResponseRedirect(reverse('foodadv:practice_test_finish_confirm'))
                ...
                if request.session['form'+question_id]:
                    ...
                    for session_question_id in session_question_ids_list:
                        ...
                        if page_num < len(session_question_ids_list):
                            ...
                            if page_num+1 == len(session_question_ids_list):
                                return HttpResponseRedirect(
                                    reverse('foodadv:practice_test_finish_confirm'))
    ...
    return render(request, 'foodadv/practice_test/practice_test.html', context)
...
def practice_test_finish_confirm(request):
    ...
    return render(request, 'foodadv/practice_test/practice_test_finish_confirm.html', context)
...
def forced_termination_ajax(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        if request.method == 'POST':
            _save_record(request=request,
                            question_info=question_info)
            return JsonResponse(data={'url': reverse('foodadv:practice_result')})
...
def answer_status(request):
    ...
    # 1. session読み込み
    if not 'session_question_ids_list' in request.session:
        # 直接 foodadv/practice_test/answer_status を叩かれた時
        raise PermissionDenied()
    session_question_ids_list = request.session['session_question_ids_list']
    ...
    return render(request, 'foodadv/practice_test/answer_status.html', context)
...
def rechallenge_mistakes(request):
    ...
    record_qs = FoodadvRecord.objects.filter(question_id__in=session_question_ids_list,
                                        user_id=request.user.id)
...
def _make_search_query_param(page_num=1,
    ...
    if favorite:
        ...
        if str(SearchFoodadvRecord.FAVORITE) in favorite:
            params.append('favorite=%s' % urllib.parse.quote(str(SearchFoodadvRecord.FAVORITE)))
    if memo:
        # memoはリストで入ってくるので、処理はmemo[0]となるので以下の書き方
        if str(SearchFoodadvRecord.MEMO):
            params.append('memo=%s' % urllib.parse.quote(str(SearchFoodadvRecord.MEMO)))
    if page_num:
...
def search(request):
    ...
    return render(request, 'foodadv/search/search.html', context)
...
def search_question_id(request, question_id):
    ...
    return render(request, 'foodadv/search/search_question_id.html', context)
...
def record_question_id(request, question_id):
    ...
    return render(request, 'foodadv/record/record_question_id.html', context)
...
def record_list(request):
    ...
    return render(request, 'foodadv/record/record_list.html', context)
...
def memos(request):
    ...
    return render(request, 'foodadv/record/memos.html', context)
...
def favorites(request):
    ...
    return render(request, 'foodadv/record/favorites.html', context)
